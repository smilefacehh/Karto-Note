...
namespace karto
{
  ...
  public:
     ...
     * 在栅格地图中，计算两个点连线之间的栅格，对应栅格位置处的pass计数加1
     * @param x0 起点x
     * @param y0 起点y
     * @param x1 终点x
     * @param y1 终点y
...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  protected:
    ...
    /**
     * 根据所有历史帧创建栅格地图
     * 1、遍历历史帧，对于每一帧计算机器人与激光点连线之间的栅格，更新穿过、击中次数
     * 2、基于栅格击中、穿过次数更新栅格状态
     * @param rScans 所有历史帧
     */
    virtual void CreateFromScans(const LocalizedRangeScanVector& rScans)
    {
      ...
      const_forEach(LocalizedRangeScanVector, &rScans)
      {
        ...
        // 处理一帧激光点，计算机器人与激光点连线之间的栅格，更新穿过、击中次数
        // 1、在栅格地图中，计算两个点连线之间的栅格，对应栅格位置处的穿过计数加1
        // 2、终点击中计数加1
        AddScan(pScan);
      }
     ...
     * @param rWorldFrom      起点
     * @param rWorldTo        终点
     * @param isEndPointValid 最大阈值范围内的点
     * @param doUpdate        是否立即更新栅格状态
     * @return
     */
    ...
    /**
     * 统计栅格击中、穿过次数，更新是否占用、未知
     * @param pCell       当前栅格
     * @param cellPassCnt 穿过的次数
     * @param cellHitCnt  击中的次数
     */
  ...
  public:
     ...
     * @param pScan           当前帧，搜索中心
     * @param angleCenter     当前帧朝向（0°朝向）
     * @param angleOffset     搜索角度范围，正负范围，这里是偏移量，范围的一半
     * @param angleResolution 搜索角度步长
     */
  ...
  private:
     ...
     * 计算当前帧激光点，在某旋转角度变换下，新的激光点位置相对于当前帧位置（机器人的位置）的坐标偏移量
     * 不管当前帧在什么位置，只要旋转角度一定，那么坐标偏移量也是确定的。
     * @param angleIndex    角度数组索引
     * @param angle         旋转角度
     * @param rLocalPoints  当前帧在局部lidar系下的激光点
     */
    void ComputeOffsets(kt_int32u angleIndex, kt_double angle, const Pose2Vector& rLocalPoints, LocalizedRangeScan* pScan)
    {
      ...
      const_forEach(Pose2Vector, &rLocalPoints)
      {
        ...
        // 这里的offset是当前lidar帧坐标系下的坐标，不是world系坐标，那么直接调用WorldToGrid，实际上计算的是该点与当前帧位置之间的坐标偏移量
        // 注意是偏移量，目的是后面在搜索空间中对当前帧位姿施加不同的x平移、y平移之后施加一个旋转的操作，不用重新计算每个搜索位置处旋转某个角度之后对应的点坐标
        // 只需要加上对应旋转角度下算好的坐标偏移量即可。
        // 注：在任意位置下，旋转一定角度的激光点坐标偏移量都是相等的。
        ...
        // 存当前旋转角度下，激光点索引 - 新点相对于原始点的网格索引偏移
